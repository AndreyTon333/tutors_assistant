...
async def process_start_command(message: Message,  state: FSMContext, bot: Bot) -> None:
    ...
    await bot.send_message(
        ...
        text='Выберите в каком режиме вы хотите протестировать бота',
        reply_markup=kb.kb_admin_user_test()
    )
...
async def process_admin_test(clb: CallbackQuery, bot: Bot, state: FSMContext):
    ...
    try:
        ...
        pass
    if clb.data == 'admin_test':
        await bot.send_message(
            chat_id=clb.message.chat.id,
            text='Приветствую Вас!\nВы являетесь администратором проекта!',
            reply_markup=kb.kb_admin_dz_lrn())
    elif clb.data == 'user_test':
        if await rq.is_learner_in_database(tg_id=clb.message.chat.id):
            await process_begin_without_state(message=clb.message, state=state)
        else:
            await bot.send_message(
                chat_id=clb.message.chat.id,
                text='Пришлите мне токен, который вы получили от администратора проекта\n\nДля генерации токена перейдите в режим Администратора проекта',
                reply_markup=kb.kb_admin_token_test())
            await state.set_state(state=Registration.state_token)
