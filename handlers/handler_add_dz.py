...
async def process_home_task(message:Message, bot:Bot):
...
async def process_add_dz_written_content_to_bd(clb: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    if answer == 'add_more':
        ...
        if 'content' in (await state.get_data()):
            ...
            logging.info(f"await state.get_data() = {await state.get_data()}")
            _ = dict_content.pop('count')
        await rq.add_content(dict_content)
        logging.info(f"--- await state.get_data() = {await state.get_data()} ")


        await clb.message.answer(text=f'Вы добавили домашнее задание "{dict_content["name_dz"]}". Вы можете продолжать добавлять ДЗ.')
        await state.clear()
        await state.set_state(state=None)
        await process_add_dz(clb = clb)
